...
import org.bbva.kidsbankingapp.utils.MyStringUtils;
...
public class BKidsBankingApp {
	...
	public Padre registrarPadre(Padre padre) throws ServiceException, SQLException {
		...
		if (MyStringUtils.isBlank(padre.getId())
				|| MyStringUtils.isBlank(padre.getNombre())
				|| MyStringUtils.isBlank(padre.getMail())
				|| MyStringUtils.isBlank(padre.getPassword())) {
	...
	public Hijo registrarHijo(Hijo hijo) throws ServiceException, SQLException {
		if (MyStringUtils.isBlank(hijo.getId())
				|| hijo.getPadre()==null 
				|| MyStringUtils.isBlank(hijo.getNombre())
				|| MyStringUtils.isBlank(hijo.getMail())
				|| MyStringUtils.isBlank(hijo.getPassword())) {
			throw new ServiceException(Constants.MANDATORY_PARAMETERS_MISSING);
		}
	...
	public List<Movimiento> getMovimientosCuenta(String cuentaid) throws ServiceException, SQLException {
		if (MyStringUtils.isBlank(cuentaid)) {
			throw new ServiceException(Constants.MANDATORY_PARAMETERS_MISSING);
		}
	...
	public List<Producto> getProductos(String nivelid) throws ServiceException, SQLException {
		if (MyStringUtils.isBlank(nivelid)) {
			throw new ServiceException(Constants.MANDATORY_PARAMETERS_MISSING);
		}
	...
	public Movimiento postCompra(Compra compra) throws ServiceException, SQLException {
		...
		if (compra.getProducto()!=null && !MyStringUtils.isBlank(compra.getProducto().getId())) {} else {
			throw new ServiceException(Constants.MANDATORY_PARAMETERS_MISSING);
		}
		if (compra.getTarjeta()!=null && !MyStringUtils.isBlank(compra.getTarjeta().getId())) {} else {
			throw new ServiceException(Constants.MANDATORY_PARAMETERS_MISSING);
		}
	...
	public Movimiento postIngreso(Movimiento movimiento) throws ServiceException, SQLException {
		// 
		if (movimiento.getCuenta() != null && !MyStringUtils.isBlank(movimiento.getCuenta().getId())) {
			throw new ServiceException(Constants.MANDATORY_PARAMETERS_MISSING);
		}
